...
def create_healthsecure_texts():
    documents_data = get_documents_data()
    os.makedirs("data/healthsecure_texts", exist_ok=True)

    for filename, data in documents_data.items():
        txt_name = os.path.splitext(filename)[0] + ".txt"
        txt_path = os.path.join("data/healthsecure_texts", txt_name)
